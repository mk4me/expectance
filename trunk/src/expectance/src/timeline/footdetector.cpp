...
{
	////FOOTPLANT_TRACE = ((Config::getInstance()->IsKey("track_footplant")) && (Config::getInstance()->GetIntVal("track_footplant")==1));
...
void FootDetector::Apply(float elapsedSeconds, TimeLineContext * timeLineContext)
{
	...
	CalSkeleton *_skel = avImpl->getOsgModel()->getCalModel()->getSkeleton();
