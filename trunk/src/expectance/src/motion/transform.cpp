...
{
    float forwardAngle = getZangle(CalQuatToQuat(origQuat));
...
void Transform::Init(const CalVector& origPos, float forwardAngle, float endForwardAngle, bool source_3dsmax)
{
    ...
    CalVector offset(-origPos.x,-origPos.y, 0); 
    setPosOffset(offset);
    
    //calculate orig forward
    CalVector vOrigForward(0,1,0);
    ...
    if (source_3dsmax)
    {
		...
		qDiffForward = QuatToCalQuat(   osg::Quat( forwardAngle , osg::Vec3d(0,1,0))    );
        
        //set orinal forward
		CalQuaternion quat = QuatToCalQuat(     osg::Quat( -forwardAngle , osg::Vec3d(0,1,0))    );
        vOrigForward *= quat;

		qEndForward = QuatToCalQuat(   osg::Quat( -endForwardAngle , osg::Vec3d(0,1,0))    );
...
float Transform::CalculateAnimForward(CalCoreAnimation* coreAnim, int iterCount, bool from_begin)
{
    ...
    for (int i = 0; i<iterCount; i++)
    {
        ...
        float rotZ = getZangle( CalQuatToQuat(frame->getRotation()) );
        angle += rotZ;

        float deg = radToDeg(rotZ);
