...
Modu³ UpdateManager jest jednym z bazowych modu³ów Generatora.  Do jego g³ównych zadañ nale¿y odœwie¿anie obiektów w ka¿dej klatce symulacji (frame) oraz rozsy³anie wiadomoœci do zainteresowanych obiektów. G³ówn¹ klas¹ modu³u jest \textit{UpdateManager}, która posiada tylko jedn¹ instancjê (Singleton). Aby obiekty mog³y byæ odœwie¿anie przez \textit{UpdateManagera} lub odbieraæ wiadomoœci musz¹ byæ pochodnymi klasy \textit{UpdateObject} oraz byæ zarejestrowanie w UpdateManager'ze.
...
Strukturê avatara mo¿na podzieliæ na kilka warstw logicznych. Hierarchia w kodzie tych warst zosta³a przedstawiona na rys \ref{rys_4}. W kolejnych podrozdzia³ach zostan¹ opisane funkcjonalnoœci jakie s¹ zwi¹zane z poszczególnymi warstwami.
...
Dane ruchów u¿ywane przez Generator to dane Motion Capture przekszta³cone do formatu Cal3d. Tylko w takim formacie dane te sa widoczne z modu³ów Generatora (nie ma mo¿liwoœci dostêpu do oryginalnego formatu zawartego w plikach motion capture - np. BVH).

\subsection{Warstwa animacji Cal3d}

Dane ruchu u¿ywane w Generatorze maj¹ formê animacji Cal3d. Odgrywanie ruchów polega na odgrywaniu tych animacji przez API zdefinowane przez engine Cal3d. Dostêp do szczegó³owych danych dla poszczególnych koœci polega na pobieraniu œcie¿ek animacji (tzw track-ów) z animacji Cal3d.  Œcie¿ki te s¹ parametryzowane czasem - mo¿na wyci¹gn¹æ dowoln¹ klatkê animacji zadaj¹c odpowiedni czas \textbf{t} z zakresu od 0 do d³ugoœci (czasu trwania) tej animacji.  Dla poœrednich wartoœci czasu \textbf{t}, które nie maj¹ zdefiniowanych wartoœci w œcie¿ce, klatki sa wyliczane z u¿yciem interpolacji pomiêdzy najblizszymi zdefiniowanymi klatkami w œcie¿ce.
...
W warstwie tej przewidziane jest tworzenie modyfikatorów, które dostosowuj¹ zdefinowan¹ TimeLine'ami sekwencjê ruchów aby poprawiæ realizm i podtrzymanie poprawnoœci fizycznych aspektów ruchu np. eliminacja œlizgania siê stóp po pod³odze, poprawne stawianie kroków podczas skrêcania lub wykorzystanie kinematyki odwrotnej.

\subsection{Warstwa 'Control'}

Warstwa 'Control' ma zapewniæ mo¿liwoœæ definiowania akcji oraz powi¹zañ pomiêdzy akcjami na wy¿szym poziomie ni¿ sama definicja sekwencji ruchu. Akcje mog¹ byæ wykonywane zgodnie ze zdarzeniami pochodz¹cymi od u¿ytkownika lub zgodnie z algorytmami sztucznej inteligencji w warstwach wy¿szych.

\subsection{Warstwa 'Think'}

W warstwie 'Think' przewidziana jest implementacja algorytmów sztucznej inteligencji, które wprowadzaj¹ autnomiczne i inteligentne zachowanie avatarów np.: omijanie przeszkód czy  wyznaczanie œcie¿ki do celu.
...
oraz jeden obiekt typu \textit{ft::TimeLineExecutor}. Aby rozpocz¹æ wykonywanie TimeLine'a przypisanego do danego avatara nale¿y wywo³aæ metodê \textit{Start()} na TimeLineExecutor'ze tego avatara. Po wywo³aniu tej metody rozpoczyna siê wykonywanie kolejno wszystkich elementów (ruchów) sk³adowych TimeLine'a.
...
W TimeLineExecutorze zdefiniowany jest zbiór stanów, pomiêdzy którymi mo¿e on przechodziæ wykonuj¹c TimeLine'a. Na rysunku \ref{rys_7} przedstawione s¹ wszystkie mo¿liwe stany jak i dozwolone przejœcia pomiêdzy nimi.
\\
...
\begin{figure}[!h]
  ...
  \end{center}
  \caption{Schemat przejœæ pomiêdzy stanami TimeLineExecutor'a}
  \label{rys_7}
\end{figure}
...
\item 'WAIT' - w tym stanie TimeLine nie zawiera ¿adnych ruchów do wykoniania (jest pusty lub wszystkie sk³adowe ruchy zosta³y ju¿ wykonane)
...
\item 'OVERLAP' - stan ten reprezentuje "zadk³adkê" - czyli blending pomiêdzy dwoma kolejnymi ruchami
\item 'FADE\_OUT' - wykonywany jest jeden ruch przy czym jego  waga w Cal3d jest interpolowana od 1 do 0
\item 'TERMINATED' - ruch zosta³ nagle zatrzymany z zewn¹trz przez metodê \textit{TimeLineExecutor::StopRequest()}. Aktualnie wykonywana animacja
 jest zakoñczona natychmiast lub z zakonczeniem aktualnej animacji (lub aktualnego cyklu dla animacji cyklicznych)
\end{enumerate}  
...
\item \textbf{currAnim} (\textit{CalAnimation*}) - animacja, która jest aktualnie wykonywana
...
\item \textbf{prevAnimDuration} ( \textit{float}) - ca³kowita d³ugoœæ (czas trwania) poprzedniej animacji
\item \textbf{currAnimTime} (\textit{float}) - aktualny czas aktualnie wykonywanej animacji
\item \textbf{currAnimDuration} (\textit{float}) - ca³kowita d³ugoœæ (czas trwania) aktualnie wykonywanej animacji
\item \textbf{prevOverlap} (\textit{float}) - czas  \textit{blendingu} zastosowanego dla poprzedniej animacji
\item \textbf{currOverlap} (\textit{float}) - czas  \textit{blendingu}, który bêdzie u¿yty pomiêdzy aktualna a nastêpn¹ animacj¹
\item \textbf{exec\_state} (\textit{int}) - aktualny stan w jakim jest TimeLineExecutor
\item \textbf{exec\_event} (\textit{int})  - informacja o ostatnim zdarzeniu wygenerowanym przez TimeLineExecutor (aktualnie s¹ mo¿liwe dwie wartoœci EXEC\_EVENT\_NONE lub EXEC\_EVENT\_STATE\_CHANGED
\item \textbf{anim\_changed} (\textit{bool}) - ma wartoœæ  \textit{true} jeœli w aktualnym odœwie¿eniu zmieni³a siê animacja
\item \textbf{anim\_new\_cycle} (\textit{bool}) - ma wartoœæ  \textit{true} jeœli w aktualnym odœwie¿eniu rozpocz¹³ siê nowy cykl jeœli w³aœnie wykonywana jest animacja cykliczna
\item \textbf{anim\_stopped} (\textit{bool}) - ma wartoœæ  \textit{true} jeœli w aktualnym odœwie¿eniu animacja siê skoñczy³a lub zosta³a zatrzymana
\end{enumerate}  

Oprócz parametrów informacyjnych TimeLineContext zawiera równie¿ parametry, które mo¿na ustawiæ z zewn¹trz:
\begin{enumerate}
\item \textbf{stop\_immediate} (\textit{bool}) - ustalenie czy zatrzymanie wykonywania TimeLine'a (przejœcie do stanu TERMINATED) ma byæ wykonane natychmiast czy po zakoñczeniu aktualnej animacji (lub aktualnego cycklu dla animacji cyklicznych)
\item \textbf{remove\_after\_execution} (\textit{bool}) - ustalenie czy obiekty TimeLineMotion maj¹ byæ usuwane z TimeLine'a po wykonaniu TimeLine'a
\end{enumerate}  
...
Wykonywanie akcji przez avatara polega na przechodzeniu pomiêdzy poszczególnymi stanami (tzw. \textit{ControlState's}).  Ka¿dy avatar posiada w³asny i niezale¿ny zbiór stanów, pomiêdzy którymi mo¿e "przechodziæ". Ka¿dy stan ControlState posiada 3 metody, które wo³ane s¹ w momencie inicjacji stanu (\textit{Init()}), wejœcia do tego stanu (\textit{Entry()}) oraz wyjœcia ze stanu (\textit{Exit()}).
\\
...
\begin{enumerate}
\item Ka¿dy obiekt który ma byæ w³¹czony do potoku renderuj¹cego musi implementowaæ interfejs (pokrywa metodê \textit{Render}) obiektu \textit{ft::SceneObject}.
\item Ka¿dy obiekt, który ma byæ w³¹czony do potoku renderuj¹cego jest musi byæ zarejestrowany przez obiekt \textit{ft::VisualizationManager} przy pomocy metody \textit{ft::VisualizationManager::AddObject} 
\item Obiekt \textit{ft::VisualizationManager} wywo³uje cyklicznie, zsynchronizowan¹ z mechanizmem GLUT, metodê \textit{ft::VisualizationManager::OnRender}, i przetwarza wszystkie zarejestrowane obiekty wywo³uj¹c metodê Render ka¿dego z nich.
\end{enumerate}  
...
\begin{enumerate}
\item Utworzyæ pliki z kodem vertex shaderów i pixel shaderów dla fragmentów lub ca³ego obiektu graficznego w katalogu \textbf{shaders}. Shadery dla pixel shaderów (fragment programy) maj¹ rozszerzenie \textit{.frag} natomiast dla vertex shaderów (vertex programy) rozszerzenie \textit{.vert}.
...
\section{System debug'ów}

Wszystkie debugi wypisywane na konsole z Generatora, które maja byæ dodane do oficjalnej wersji w SVN-ie, powinny byæ wypisywane z u¿yciem metod zdefiniowanych w klase \textit{ft::Debug} z pakietu \textit{/utility}.
\\

Do wypisywania tekstu zaleca siê uzywanie polecania  \textit{\_dbg} (zdefniowanego w pliku  \textit{/utility.debug.h}) zamiast standardowych polecen  \textit{cout},  \textit{cerr} lub  \textit{printf}.  
\\

Dodatkowo zaleca siê u¿ywanie warunków z u¿yciem flag zdefniowanych w klasie \textit{ft::Debug} (np. flaga \textit{ERR} definiuje wypisywania debugow zwiazanych z bledami) . 
\\
...
\subsection{B³êdy i ostrze¿enia}

Do wypisywania bledow i warningow sluza specjalne flagi \textit{Debug:ERR} oraz \textit{Debug::WARN}. Sa one typu bool i kazde wypisanie bledu lub ostrzezenia nale¿y ograniczyc warunkiem z uzyciem tych flag.  Dodatkowo ztefiniowane sa slowa kluczowe dla bledow i ostrzezen, których warto uzywac w celu ulatwienia w przeszukiwaniu loga: \textit{Debug::ERR\_STR} oraz \textit{Debug::WARN\_STR}.
\\
...
\subsection{Poziomy debugów dla modulów}

Oprócz bledów i ostrze¿eñ zosta³y zdefiniowane poziomy debugów dla modu³ów. Dla ka¿dego modulu (lub logicznej czesci kodu zwiazanej z dana funkcjonalnoscia) nale¿y dodac statyczna zmienna typu int do klasy \textit{Debug}. Nastepnie wypisujac debugi dla danego modulu nale¿y tej zmiennej uzywac sprawdzajac czy zostal ustawiony wystarczajacy poziom debugow dla modulu np.:
\textbf{ 
\begin{flushleft} \small
	if (Debug::CONTROL>1)
...
Powyzszy debug zostanie wypisany jeœli poziom debugow \textit{CONTROL} zostanie ustawiony na wyzszy ni¿ 1 (czyli np. 2).
...
Teraz gdy ustawimy w pliku application.cfg wpis np. \textit{DEBUG\_CONTROL = 2} to poziom debugów \textit{CONTROL} zostanie ustawiony na 2.
\subsection{Debugi dla projektu 'genenerator'}
Poziomy debugów dla modulów w projekcie 'generator' warto dodawaæ w klasie \textit{GenDebug} (pakiet \textit{/app}). Dotyczy to równie¿ u¿ywania systemu debugów w kodzie - czyli
wszêdzie tam gdzie u¿ylibysmy klasy  \textit{Debug} powinnismy u¿yæ klasy  \textit{GenDebug}. Zalet¹ u¿ywania klasy  \textit{GenDebug} jest to, ¿e nie musmy zmieniaæ/kompilowaæ ¿róde³ projektu 'engine'.
